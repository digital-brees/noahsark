...
- **GREY BAR = a CSS stacking bug, now fixed.** The `.bridge`d `.svc-intro` panel lifts `-52px` up onto the hero. Its bg is porcelain `--paper` (#FCFBFA), BUT the hero's dark scrim `.carely-hero__scrim` (`rgba(26,21,38,.52)`, `z-index:1`) was painting ON TOP of the panel in the overlap zone, because `.svc-intro` was `z-index:auto` — and positioned elements with `z-index:1+` always paint above `z-auto` siblings regardless of DOM order. Porcelain under a 52% dark scrim blends to ~`rgb(135,132,138)` = exactly that mid-grey. The 2026-06-12 "fix" (tightening hero padding) only hid it; the scrim was still winning. **Real fix:** added `z-index:1` to `.bridge > section:not(:first-of-type)` — equal to the scrim, so DOM order wins (panel comes later → paints over scrim), and still below hero text (`.carely-hero__inner` is `z-index:2`) so nothing readable is covered. Verified: panel now paints above scrim, grey gone, clean white rounded ledge.
- **BAR TOO CLOSE TO BUTTON.** Gap was only 8px (button bottom 678, panel top 686). Root cause: the 2026-06-12 grey-band "fix" had tightened `.carely-hero__inner` padding-bottom 76→60px, pulling the button down toward the lift. Now that the grey is fixed properly, **restored breathing room:** padding-bottom 60→**104px** (desktop), 52→**76px** (mobile ≤860px). Verified gap now **52px**.
- **Cache:** `site.css?v=60 → v=61` across all 21 HTML pages (shared CSS change — the `.bridge` z-index fix also benefits the team page, which uses the same class).
- All copy + the rest of the design system untouched.
